...
def daily_schedule_Update_Answer_Entities(CheckAnsweredTimer: func.TimerRequest):
    ...
    try:
        all_qna_pairs = chat_history_service.get_question_answer_pairs()
        ...
        for pair in all_qna_pairs:
            ...
            if answer_message_id:
                is_answer = chat_history_service.CheckQuestionAnswered(
                    question.get("content"), 
                    answer.get("content"), 
                )
                updated_data = {MessagePropertyName.IS_ANSWERED.value: is_answer}
                chat_history_service.update_message(updated_data, message_ids=[answer_message_id, question_message_id])
