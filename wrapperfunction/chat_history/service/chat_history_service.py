...
async def upload_documents(files, bot_name,  request: Request,conversation_id: Optional[str] = None ):
    try:
        ...
        if not conversation_id:
            ...
            user_message_entity = MessageEntity(content=content, conversation_id=conversation_id, role=Roles.User.value, context="", type=MessageType.Document.value,display_in_chat=False)
            client_details = extract_client_details(request)
...
async def start_video_chat(video_id: str, request: Request, bot_name: str,language:str):
    try:
        ...
        res_data=response.get("data",{}).get("res_data",{})
        content = res_data.get("videos", [])
        client_details = extract_client_details(request)
        conversation_id = str(uuid.uuid4())
