...
def detection(text,criteria:Optional[str] = None):
    if not criteria:
        ...
        "Return the results as a list of page numbers without any other text:\n\n"
        f"{text}"
    )
    else:
        print(criteria)
        prompt = (
    f"The following text is extracted from file, Identify the best practice to split this file based on {criteria}. "
    "Return the results as a list of page numbers without any other text. "
    "If no criteria is specified, return an empty list:\n\n"
    f"{text}"
)   
    try:
        response = openaiconnector.client.chat.completions.create( 
...
def split_pdf(input_pdf_path, criteria):
    try:
        ...
        for i in range(len(pages) - 1):
            writer = PdfWriter()
            for j in range(pages[i] - 1, pages[i + 1] - 1):
        ...
        return {"status": "success", "message": "Zip file uploaded successfully","url":f"{response.url}"}
