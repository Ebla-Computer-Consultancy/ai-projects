...
def summarize_with_openai(content: str, bot_name: str, language: str):
    try:
        response = openai_connector.chat_completion(
            ...
            chat_history=[{"role": "system", "content": f"Summarize the following content in {language}, if possible. If summarization is not possible, respond with 'No transcript available.'"}, {"role": "user", "content": content}]
        )
