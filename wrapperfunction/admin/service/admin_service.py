from wrapperfunction.admin.integration.crawl_integration import run_crawler, process_and_upload, upload_files_to_blob,delete_blobs_base_on_metadata, delete_base_on_subfolder, edit_blob_by_new_jsonfile, split_json_file, update_json_with_pdf_text, scrape_csv
...
def scrape(file_path: str,container_name):
    if file_path.endswith('.csv'):
        results = scrape_csv(file_path,container_name)
        process_and_upload("./"+container_name+".json","./export/", "  ")
        return results
    else:
        raise HTTPException(status_code=400, detail="Please provide a valid CSV file path.")
