...
def scrape_csv(csv_file: UploadFile, container_name: str):
    buffer = BytesIO(csv_file.file.read())
...
def process_and_upload(
    ...
    with open(filepath, "r", encoding="utf-8") as file:
        ...
        for i, item in enumerate(data):
            ...
            individual_filepath, individual_filename = filter_json2file(
                item, folder_path
            )
            if doc:
                push_blob(
...
def push_blob(
    ...
    blob_client = blob_service_client.get_blob_client(
        container=container_name, blob=f"{SUBFOLDER_NAME}/{individual_filename}"
    )
    encode = "utf-8"
